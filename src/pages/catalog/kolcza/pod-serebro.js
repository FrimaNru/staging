...
export default function PodSerebroKolcza({ products, popularProducts }) {
    return (
        <>
            <Head>
            ...
            <center>
                <main>
                    <Header />
                    <Catalog initialPage={1} initialProducts={products} initialType="Кольца" popularProducts={popularProducts} />
                    <Footer />
                </main>
            </center>
        </>
...
export async function getServerSideProps({ query }) {
    ...
    const products = await getFilteredProducts({
        subcategoryPath: '/kolcza/pod-serebro',
        productType: 'ring',
        text,
        filter
    });
