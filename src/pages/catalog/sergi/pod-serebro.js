...
export default function SilverEarringsPage({ products, popularProducts }) {
    const canonicalUrl = getCanonicalUrl('/catalog/sergi/pod-serebro');

    return (
        <>
            <Head>
                <title>Серьги под серебро – цена, купить в Mi Alegria</title>
                <meta name="description" content="Купить серьги под серебро - элитная бижутерия в Москве в интернет-магазине Mi Alegria ✔ Актуальный дизайн для любого возраста, стиля и случая ✔ Подобрать и заказать серьги под серебро, а также другие украшения можно на нашем сайте" />
...
export async function getServerSideProps({ query }) {
    ...
    const products = await getFilteredProducts({
        subcategoryPath: '/sergi/pod-serebro',
